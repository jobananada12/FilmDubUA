...
class ClipTimeline(QWidget):
    ...
    def __init__(self):
        super().__init__(); self.setMinimumHeight(190); self.clips=[]; self.duration=60000; self.current=0; self.drag=None; self.selected_index=-1; self.setMouseTracking(True)
    def set_duration(self,ms): self.duration=max(1,ms); self.update()
    def set_current(self,ms): self.current=ms; self.update()
    def set_clips(self,clips): self.clips=clips; self.update()
    def x_for(self,ms):
    ...
    def paintEvent(self,e):
        p=QPainter(self); p.fillRect(self.rect(),QColor('#15171b')); l=55; w=max(1,self.width()-75)
        ...
        for i,c in enumerate(self.clips):
            ...
            p.setBrush(QBrush(QColor('#3b82f6') if sel else QColor('#2563eb'))); p.setPen(QPen(QColor('#93c5fd') if sel else QColor('#60a5fa'),2)); p.drawRoundedRect(rect,6,6)
            p.setPen(Qt.white); p.drawText(rect.adjusted(8,5,-8,-5),Qt.AlignVCenter,c['text'].replace('\n',' ')[:42])
            p.setPen(QPen(QColor('#dbeafe'),4)); p.drawLine(int(x1),57,int(x1),123); p.drawLine(int(x2),57,int(x2),123)
    ...
    def mousePressEvent(self,e):
        ...
        for i,c in enumerate(self.clips):
            ...
            if abs(x-x1)<12:self.drag=('start',i); self.selected_index=i; self.selected.emit(i); self.update(); return
            if abs(x-x2)<12:self.drag=('end',i); self.selected_index=i; self.selected.emit(i); self.update(); return
            if x1<=x<=x2 and 55<=e.position().y()<=130:self.selected_index=i; self.selected.emit(i); self.update(); return
...
class FilmDubUA(QMainWindow):
    ...
    def build_ui(self):
        ...
        for text,fn in [('🎬 Відкрити відео',self.open_video),('+ Репліка',self.add_clip),('Видалити',self.delete_clip),('🎙 Експорт MP4',self.export)]:
        ...
        g=QGroupBox('Мікшер'); gl=QVBoxLayout(g); gl.addWidget(QLabel('Оригінальний звук відео')); self.orig=QSlider(Qt.Horizontal); self.orig.setRange(0,100); self.orig.setValue(25); gl.addWidget(self.orig); self.orig_label=QLabel('25%'); gl.addWidget(self.orig_label); self.orig.valueChanged.connect(lambda v:self.orig_label.setText(f'{v}%')); gl.addWidget(QLabel('Гучність озвучки')); self.dub=QSlider(Qt.Horizontal); self.dub.setRange(50,200); self.dub.setValue(100); gl.addWidget(self.dub); self.dub_label=QLabel('100%'); gl.addWidget(self.dub_label); self.dub.valueChanged.connect(lambda v:self.dub_label.setText(f'{v}%')); rl.addWidget(g); rl.addWidget(QLabel('Голос Windows')); self.voice=QComboBox(); self.load_voices(); rl.addWidget(self.voice); split.addWidget(right); split.setSizes([850,380]); main.addWidget(split,1); main.addWidget(QLabel('ТАЙМЛАЙН — перетягуй краї репліки мишкою')); main.addWidget(self.timeline); self.status=QLabel('Готово'); main.addWidget(self.status)
    ...
    def export(self):
        ...
        if not self.clips:return QMessageBox.warning(self,'FilmDubUA','Додай репліки.')
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                orig=float(self.orig.value())/100; dub=float(self.dub.value())/100
                fs.append(f'[0:a]volume={orig}[original]'); fs.append(f'[voice]volume={dub}[dubbed]'); fs.append('[original][dubbed]amix=inputs=2:duration=longest:dropout_transition=0[mix]')
                cmd=['ffmpeg','-y']+ins+['-filter_complex',';'.join(fs),'-map','0:v:0','-map','[mix]','-c:v','copy','-c:a','aac','-shortest',out]
