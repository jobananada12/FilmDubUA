...
class FilmDubUA(QMainWindow):
    ...
    def _build(self):
        ...
        bar=QHBoxLayout(); open_btn=QPushButton('🎬 Додати фільм'); open_btn.clicked.connect(self.open_video); export_btn=QPushButton('📱 Експорт 9:16'); export_btn.clicked.connect(self.export); bar.addWidget(open_btn); bar.addWidget(export_btn); bar.addStretch(); self.status=QLabel('Готово'); bar.addWidget(self.status); main.addLayout(bar)
        ...
        self.timeline=Timeline(); self.timeline.positionChanged.connect(self.player.seek); self.player.player.durationChanged.connect(self.timeline.set_duration); self.player.player.positionChanged.connect(self.timeline.update_position); main.addWidget(self.timeline)
