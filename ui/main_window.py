...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        prepare = QPushButton('🧩 3. Додати уривок у монтаж')
        ...
        montage_box = QGroupBox('🎞️ Монтажна доріжка')
        montage_layout = QVBoxLayout(montage_box)
        self.montage_list = QListWidget()
        self.montage_list.setMinimumHeight(110)
        self.montage_list.currentRowChanged.connect(self.select_montage_clip)
        montage_layout.addWidget(self.montage_list)
        montage_buttons = QHBoxLayout()
        remove = QPushButton('🗑 Видалити кліп')
        remove.clicked.connect(self.remove_montage_clip)
        clear = QPushButton('🧹 Очистити монтаж')
        clear.clicked.connect(self.clear_montage)
        montage_buttons.addWidget(remove)
        montage_buttons.addWidget(clear)
        montage_layout.addLayout(montage_buttons)
        main.addWidget(montage_box, 0)
    ...
    def selection_changed(self, start_ms, end_ms):
        ...
        self.montage_ready = bool(self.montage_clips)
        self.montage_path = ''
        self.background_path = ''
        self.previewing_selection = False
        ...
        if self.selection_ready:
            ...
            self.montage_status.setText(
                f'🎞️ У монтажі: {len(self.montage_clips)} кліп(ів). Можна додати вибраний уривок.'
            )
            self.status.setText('✅ Початок і кінець запамʼятовано.')
    ...
    def preview_selection(self):
        ...
        self.status.setText(f'▶ Перегляд: {self.format_ms(self.selection_start_ms)} → {self.format_ms(self.selection_end_ms)}')
    ...
    def on_position_changed(self, position_ms):
        ...
        if self.previewing_selection and position_ms >= self.selection_end_ms:
            ...
            self.previewing_selection = False
            self.status.setText(f'✅ Перегляд завершено: {self.format_ms(self.selection_start_ms)} → {self.format_ms(self.selection_end_ms)}')
    ...
    def prepare_montage(self):
        ...
        output_path = self.montage_temp_dir / f'clip_{len(self.montage_clips) + 1:03d}.mp4'
        self.montage_ready = False
        self.montage_path = ''
        self.status.setText('⏳ Створюю кліп для монтажної доріжки...')
        self.montage_worker = MontageWorker(
            self.movie_path, self.selection_start_ms, self.selection_end_ms, output_path
        )
    ...
    def montage_done(self, path):
        ...
        self.montage_clips.append({
            'path': path,
            'start_ms': self.selection_start_ms,
            'end_ms': self.selection_end_ms,
        })
        self.montage_ready = True
        index = len(self.montage_clips)
        self.montage_list.addItem(
            f'Кліп {index}  |  {self.format_ms(self.selection_start_ms)} → '
            f'{self.format_ms(self.selection_end_ms)}  |  {Path(path).name}'
        )
        self.montage_list.setCurrentRow(index - 1)
        self.montage_status.setText(f'🎞️ У монтажі: {index} кліп(ів)')
        self.status.setText('✅ Кліп реально створено й додано на монтажну доріжку.')
        self.player.open(path)
    ...
    def montage_failed(self, error):
        self.montage_ready = bool(self.montage_clips)
        self.montage_status.setText('Монтаж: кліп не створено')
        self.status.setText(f'❌ Не вдалося створити кліп: {error}')

    def select_montage_clip(self, row):
        if not (0 <= row < len(self.montage_clips)):
            return
        path = self.montage_clips[row]['path']
        if Path(path).exists():
            self.player.open(path)
            duration = self.montage_clips[row]['end_ms'] - self.montage_clips[row]['start_ms']
            self.timeline.set_duration(duration)
            self.timeline.update_position(0)
            self.status.setText(f'▶ Відкрито кліп {row + 1} з монтажної доріжки')

    def remove_montage_clip(self):
        row = self.montage_list.currentRow()
        if not (0 <= row < len(self.montage_clips)):
            return
        self.montage_clips.pop(row)
        self.montage_list.takeItem(row)
        for i in range(self.montage_list.count()):
            clip = self.montage_clips[i]
            self.montage_list.item(i).setText(
                f'Кліп {i + 1}  |  {self.format_ms(clip["start_ms"])} → '
                f'{self.format_ms(clip["end_ms"])}  |  {Path(clip["path"]).name}'
            )
        self.montage_ready = bool(self.montage_clips)
        self.montage_status.setText(
            f'🎞️ У монтажі: {len(self.montage_clips)} кліп(ів)' if self.montage_clips else 'Монтаж: порожній'
        )
        self.status.setText('🗑 Кліп видалено з монтажу')
    ...
    def clear_montage(self):
        ...
        self.montage_path = ''
        self.background_path = ''
        self.montage_status.setText('Монтаж: порожній')
        self.status.setText('🧹 Монтаж очищено')
    ...
    def separate_selected_voice(self):
        ...
        if not self.montage_ready or not self.montage_path:
            self.status.setText('Спочатку додайте уривок у монтаж')
            return
        if self.worker and self.worker.isRunning():
            return
        self.status.setText('🤖 AI: готую аудіо вибраного уривка та відділяю голоси...')
        self.worker = SeparationWorker(self.movie_path, self.selection_start_ms, self.selection_end_ms)
    ...
    def separation_done(self, background):
        ...
        self.montage_status.setText('🎵 Основа готова: відео + фон без оригінального вокального шару. Далі — нові репліки.')
