...
from core.tts import windows_voices, synthesize_ukrainian, TEMP_ROOT
from core.dubbing import mix_dialogue, mix_dialogues

class DialogueEditor(QWidget):
    """Dialogue editor with local Ukrainian Piper TTS."""
    def __init__(self, parent=None):
        ...
        self.voice=QComboBox(); self.rate=QSpinBox(); self.rate.setRange(80,300); self.rate.setValue(170); self.rate.setSuffix(' сл/хв')
        ...
        voice_row=QHBoxLayout(); voice_row.addWidget(QLabel('Голос')); voice_row.addWidget(self.voice,1); voice_row.addWidget(QLabel('Швидкість')); voice_row.addWidget(self.rate); voice_row.addWidget(generate); voice_row.addWidget(mix); voice_row.addWidget(mix_all)
    ...
    def add_dialogue(self):
        self.track.add(Dialogue(self.start.value(),self.end.value(),self.character.text(),self.text.text())); self.refresh(); self.list.setCurrentRow(len(self.track.items)-1)
    ...
    def load_current(self,row):
        if 0<=row<len(self.track.items):
            ...
            self.character.setText(d.character); self.text.setText(d.text); self.start.setValue(d.start_ms); self.end.setValue(d.end_ms)
    ...
    def generate_voice(self):
        ...
        try:
            out_dir=TEMP_ROOT/'tts'; out_dir.mkdir(parents=True,exist_ok=True); out=out_dir/f'dialogue_{row+1:03d}.wav'
            # Always use the real Ukrainian neural voice. Windows SAPI is kept only as an optional legacy list item.
            synthesize_ukrainian(d.text,str(out),rate=self.rate.value(),volume=d.volume)
            d.audio_path=str(out); self.refresh(); self.list.setCurrentRow(row); self.status_message(f'✅ Українську озвучку створено: {out}')
