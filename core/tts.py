...
def available_engines():
    engines=['piper-uk']
    try:
        import pyttsx3; engines.append('pyttsx3')
    except ImportError: pass
    return engines

def ensure_ukrainian_model() -> Path:
    ensure_temp_dirs(); config=Path(str(UKRAINIAN_MODEL)+'.json')
    if UKRAINIAN_MODEL.exists() and config.exists(): return UKRAINIAN_MODEL
    try:
    ...
    except Exception as exc: raise TTSError(f'Не вдалося завантажити українську модель Piper: {exc}')
    if not UKRAINIAN_MODEL.exists() or UKRAINIAN_MODEL.stat().st_size < 1_000_000: raise TTSError('Українська модель Piper завантажилась некоректно')
...
def _apply_pitch(src: Path, dst: Path, semitones: float) -> None:
    ...
    filt=f'asetrate=sample_rate={factor:.8f}*sample_rate,aresample=22050,atempo={1.0/factor:.8f}'
    cmd=['ffmpeg','-y','-i',str(src),'-filter:a',filt,'-c:a','pcm_s16le',str(dst)]
    proc=subprocess.run(cmd,capture_output=True,text=True)
    if proc.returncode != 0: raise TTSError(proc.stderr[-3000:] or 'Не вдалося змінити тембр голосу')
...
def synthesize_ukrainian(text: str, output_wav: str, rate: int = 170, volume: float = 1.0, profile: str = 'neutral') -> str:
    ...
    model=ensure_ukrainian_model(); out=Path(output_wav).resolve(); out.parent.mkdir(parents=True,exist_ok=True)
    p=VOICE_PROFILES.get(profile,VOICE_PROFILES['neutral']); effective_rate=int(max(80,min(300,rate*p['rate'])))
    try:
        from piper import PiperVoice,SynthesisConfig
        voice=PiperVoice.load(str(model)); length_scale=max(0.55,min(1.55,170.0/effective_rate))
        ...
        with wave.open(str(raw),'wb') as wav_file: voice.synthesize_wav(text.strip(),wav_file,syn_config=syn_config)
        _apply_pitch(raw,out,float(p['pitch']))
        try: raw.unlink()
        except OSError: pass
    except Exception as exc: raise TTSError(f'Український Piper TTS помилка: {exc}')
    if not out.exists() or out.stat().st_size==0: raise TTSError('Piper не створив WAV-файл')
    return str(out)
...
def windows_voices():
    ...
    except Exception as exc: raise TTSError(f'Не вдалося отримати голоси Windows: {exc}')
